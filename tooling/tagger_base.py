...
class RecordingStore:
    ...
    def __init__(self, base_path, highlights, db_connection):
        self.segment_base_path = base_path
        self.highlights = highlights
        self.db_connection = db_connection

    def get_recording(self, recording_id):
        return librosa.load(f'{self.segment_base_path}/{recording_id}.m4a', sr=None)
    ...
    def get_description(self, recording_id):
        try:
            ...
            result = cur.fetchone()
            return f'Recording {recording_id} from {result[0]} at {result[1]} time {result[2]}'
...
class TagHandler:
        ...
        if recording_id in self.tag_cache:
            ...
            try:
                sql = ''' SELECT start_time_seconds, finish_time_seconds, what FROM dennis_tags
                          WHERE dennis_tags.recording_id = ?  '''
                cur = self.db_connection.cursor()
                cur.execute(sql, [int(recording_id)])
                tags = cur.fetchall()
    ...
    def hatch_and_color(self, code):
        raise NotImplementedError("Please Implement this method")
        return (None, None)

    def legend_elements(self):
        raise NotImplementedError("Please Implement this method")
        return [Patch(facecolor='orange', edgecolor='b', hatch='//', label='Morepork'),
                           Patch(facecolor='orange', edgecolor='b', hatch='xx', label='Maybepork'),
                           Patch(facecolor='orange', edgecolor='b', hatch='\\\\', label='Notpork')]


class SpectrogramViewer:
    """
    Spectrogram viewer/tagger.

    Attributes:
        tag_handler: tag reading, writing, and display implementation
        data_store: recording information implementation
        recordings: list of recordings to be displayed
        spec_config: dict of parameters
            full_freq_min: minimum frequency band for full spectrogram display
            full_freq_max: maximum frequency band for full spectrogram display
            zoom_freq_min: minimum frequency band for full spectrogram display
            zoom_freq_max: maximum frequency band for full spectrogram display
            spec_sample_size: spectrogram sample width in seconds (reciprocal is frequency band Hz)
            spec_sample_advance: spectrogram sample advance in seconds (overlapped if less than sample size)
            select_color: color used for zoom selection (default override)
            highligh_color: color used for highlight (default override)
    """
    ...
    clear_button_ax = figure.add_axes([0.10, 0.05, 0.14, 0.03])
    clear_button = Button(clear_button_ax, 'Clear')
    morepork_button_ax = figure.add_axes([0.38, 0.05, 0.14, 0.03])
    morepork_button = Button(morepork_button_ax, 'Morepork')
    maybepork_button_ax = figure.add_axes([0.53, 0.05, 0.14, 0.03])
    maybepork_button = Button(maybepork_button_ax, 'Maybepork')
    notpork_button_ax = figure.add_axes([0.68, 0.05, 0.14, 0.03])
    notpork_button = Button(notpork_button_ax, 'Notpork')
    ...
    def __init__(self, tag_handler, data_store, recordings, spec_config):
        self.tag_handler = tag_handler
        self.data_store = data_store
        self.num_bands = bands
        self.freq_min = freqmin
        self.freq_max = freqmax
        self.recordings_list = recordings
        self.full_spectrum_ax.legend(handles=tag_handler.legend_elements())
        self.select_rect = Rectangle((0, 0), 0, bands, fill=False)
        self.select_rect.set_linewidth(2.0)
        self.select_rect.set_color(select_color)
        self.highlight_rect = Rectangle((0, 0), 0, bands, fill=False)
        self.highlight_rect.set_linewidth(2.0)
        self.zoom_spectrum_ax.add_patch(self.highlight_rect)
        self.zoom_selection_rect = Rectangle((0, 0), 0, bands, fill=False, color='k')
        self.zoom_spectrum_ax.add_patch(self.select_rect)
        self.full_spectrum_ax.add_patch(self.zoom_selection_rect)
        self.highlight_box_color = highlight_color
        self.highlight_rect.set_color(highlight_color)
        ...
        self.maybepork_button.on_clicked(self.tag_maybepork)
    ...
    def button_builder(self, buttons):
        '''Add row of buttons to figure
        buttons is list of tuples (title, tag)'''

    def selection_time_span(self):
        if self.mouse_start != None and self.mouse_end != None:
            start = (min(self.mouse_start, self.mouse_end) + self.zoom_start_column) / self.columns_per_second
            end = (max(self.mouse_start, self.mouse_end) + self.zoom_start_column) / self.columns_per_second
            return (start, end)
        else:
            return (0, 0)

    def play_selection(self, frames, sr, adjust):
        ratio = sr / self.columns_per_second
    ...
    def play_filtered(self, event):
        filtered = self.butter_bandpass_filter(self.recording_frames, self.freq_min, self.freq_max, self.recording_rate, order=7)
    ...
    def on_press(self, event):
        ...
        if event.xdata and event.inaxes == self.zoom_spectrum_ax:
            ...
            self.set_zoom(event.xdata / self.columns_per_second)
            self.set_zoom_tags()
        else:
    ...
    def tag_maybepork(self, event):
        self.add_tag('maybepork')

    def tag_notpork(self, event):
        self.add_tag('notpork')
    ...
    def plot_spectrogram(self, spec, offset, image,  axis):
        ...
        numseconds = int((spec.shape[1] + 1) / self.columns_per_second)
        position_adjust = 1 / self.columns_per_second
        seconds_scale = 1
        freq_mult = 10
        if numseconds > 10:
            seconds_scale = 5
        ...
        ## create y axis
        freqstep = int((self.freq_max - self.freq_min) / spec.shape[0])
        ylabels = [i for i in range(self.freq_min, self.freq_max + 1, freqstep * freq_mult)]
        ...
        first_tick = (start_second - offset) * self.columns_per_second - position_adjust
        num_ticks = int((spec.shape[1] / self.columns_per_second + offset - start_second) / seconds_scale) + 1
        xticks = [i * self.columns_per_second * seconds_scale + first_tick for i in range(0, num_ticks)]
        print(f'spectrogram length {spec.shape[1]} ({spec.shape[1]/self.columns_per_second} seconds), xticks={xticks}')
    ...
    def time_to_column(self, time):
        return int(time * self.columns_per_second)

    def time_to_upper_column(self, time):
        return math.ceil(time * self.columns_per_second)

    def time_to_zoom(self, time):
        return self.time_to_column(time) - self.zoom_start_column

    def set_zoom(self, instant):
        start_index = max(0, int((instant - zoom_seconds / 2) * self.columns_per_second))
        end_index = start_index + int(zoom_seconds*self.columns_per_second) + 1
        if end_index > self.adjusted_spectrogram.shape[1]:
            end_index = self.adjusted_spectrogram.shape[1]
            start_index = end_index - int(zoom_seconds*self.columns_per_second) - 1
        ...
        self.zoom_image = self.plot_spectrogram(slice, start_index / self.columns_per_second, self.zoom_image, self.zoom_spectrum_ax)
        self.mouse_end = None
        ...
        self.zoom_spectrum_ax.add_patch(self.highlight_rect)
        self.set_zoom_tags()
        self.conn_patch_left = ConnectionPatch(xyA=(start_index,0), xyB=(0,self.bands), coordsA='data', coordsB='data',
                                               axesA=self.full_spectrum_ax, axesB=self.zoom_spectrum_ax)
    ...
    def set_zoom_tags(self):
        for tag in self.tags:
            ...
            if left < self.zoom_end_column and right > self.zoom_start_column:
                ...
                hatch, color = self.tag_handler.code_to_hatch_and_color[tag[2]]
                rect = Rectangle((zoom_left, 0), zoom_right - zoom_left, self.num_bands, hatch=hatch, edgecolor=color,
    ...
    def set_full_highlights(self):
        for i in range(len(self.full_highlight_texts), len(self.highlights)):
            ...
            rect.set_color(self.highlight_box_color)
            rect.set_linewidth(2.0)
        ...
        for (highlight, text, patch) in zip(self.highlights, self.full_highlight_texts, self.full_highlight_patches):
            ...
            patch.set_bounds((left, 0, width, self.num_bands))
            patch.fill = True
            self.full_spectrum_ax.add_patch(patch)
            text.set_text(str(highlight[4]))
            text.set_position((left + width / 2, self.num_bands * 5 / 6))
    ...
    def set_recording(self, position):
        ...
        npspec = np.abs(stft)[int(self.freq_min / 10):int(self.freq_max / 10)]
