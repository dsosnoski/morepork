...
save_samples = True
sample_save_path = '/home/dennis/projects/timserver/save-samples'
        ...
        if recording_id in self.tag_cache:
            ...
            try:
                ...
                if training_data:
                    ...
                    print(f'retrieved training tags {tags}')
                else:
                    ...
                    cur.execute(sql, [int(recording_id)])
                    tags = list(cur.fetchall())
                    print(f'retrieved test tags {tags}')
...
class SpectrogramViewer:
    ...
    def on_press(self, event):
        ...
        if event.xdata and event.inaxes == self.zoom_spectrum_ax:
            ...
            time = event.xdata / persec
            self.set_zoom(time)
            self.set_zoom_tags()
            if save_samples:
                path = f'{sample_save_path}/{self.recording_id}-{time:0.1f}.pk'
                self.save_sample(time, path)
        else:
    ...
    def clear_tag(self, event):
        ...
        for tag in self.tags:
            if tag[0] <= end and tag[1] >= start:
                print(f'removing tag {tag[0]}-{tag[1]} from recording {self.recording_id}')
    ...
    def set_full_highlights(self):
        ...
        for (highlight, text, patch) in zip(self.highlights, self.full_highlight_texts, self.full_highlight_patches):
            ...
            if left - last < 30:
                ...
                last = left
                top = bands * 5 / 6
            text.set_position((left + width / 2, top))
...
try:
    sql = ''' SELECT * FROM liklihoods
                      ORDER BY recording_id, start_second  '''
    # if training_data:
    #     sql = ''' SELECT * FROM training_errors
    #               ORDER BY recording_id, start_time  '''
    #     # sql = ''' SELECT * FROM dennis_february
    #     #           ORDER BY recording_id, slice_start_time  '''
    # else:
    #     sql = ''' SELECT * FROM dennis_march
    #               ORDER BY recording_id, slice_start_time  '''
#     sql = '''SELECT * FROM dennis_february
# INNER JOIN recordings ON dennis_february.recording_id = recordings.recording_id
# WHERE   recordingDateTimeNZ > '2020-04-15'
# ORDER BY recording_id, slice_start_time'''
#     sql = ''' SELECT recording_id FROM training_validation_test_data
#               WHERE recordingDateTimeNZ > '2020-01' AND recordingDateTimeNZ < '2020-02' AND what = 'dove'  '''
#     sql = ''' SELECT * FROM dennis_tags WHERE what='morepork'
...
for tuple in tags:
    ...
    has_tags[recording_id] = True
recording_highlights = {}
recording_number_of_times = {}
# for tuple in highlights:
#     # if tuple[3] > 1:
#     recording_id = tuple[1]
#     start_time = tuple[2]
...
for tuple in highlights:
    ...
    start_time = tuple[3]
    end_time = tuple[4]
    highlight = (tuple[2], recording_id, start_time, end_time, tuple[2])
    if recording_id in recording_number_of_times:
        recording_highlights[recording_id].append(highlight)
        # recording_number_of_times[recording_id] += tuple[4]
        recording_number_of_times[recording_id] += 1
    else:
        recording_highlights[recording_id] = [highlight]
        #recording_number_of_times[recording_id] = tuple[4]
...
recordings = list(recording_number_of_times.keys())
# mapped_recordings = {'recording_id': [], 'start_time': [], 'end_time': [], 'count_wrong': [], 'prediction': []}
# for id in recordings:
#     for highlight in recording_highlights[id]:
#         mapped_recordings['recording_id'].append(id)
#         mapped_recordings['start_time'].append(highlight[2])
#         mapped_recordings['end_time'].append(highlight[3])
#         mapped_recordings['count_wrong'].append(highlight[4])
#         mapped_recordings['prediction'].append(highlight[0])
# df = pd.DataFrame(mapped_recordings)
# print(df.head())
# with open('/home/dennis/projects/timserver/test-errors.csv','w') as f:
#     df.to_csv(f)
viewer = SpectrogramViewer(TagHandler(db_connection), RecordingStore(segment_base_path, recording_highlights, db_connection), recordings, 0)
