...
class SpectrogramViewer:
    ...
    def set_zoom_tags(self):
        for tag in self.tags:
            ...
            if left < self.zoom_end_column and right > self.zoom_start_column:
                ...
                hatch, color = code_to_hatch_and_color[tag[2]]
    ...
    def set_full_highlights(self):
        ...
        for (highlight, text, patch) in zip(self.highlights, self.full_highlight_texts, self.full_highlight_patches):
            ...
            text.set_position((left + width / 2, bands * 5 / 6))
...
#     sql = '''SELECT dennis_tags.ID, dennis_tags.recording_id, dennis_tags.start_time_seconds, dennis_tags.what FROM dennis_tags
# ORDER BY recording_id, start_time_seconds'''
    sql = '''SELECT recording_id FROM recordings
     WHERE recordingDateTimeNZ > '2020-02' AND recordingDateTimeNZ < '2020-03' AND device_super_name = 'Hammond_Park' AND recordings.nightRecording = "true"
     ORDER BY recording_id DESC'''
    #     sql = '''SELECT * FROM dennis_february
    # INNER JOIN recordings ON dennis_february.recording_id = recordings.recording_id
    # WHERE recordings.nightRecording = "false"
    # ORDER BY recording_id, slice_start_time'''
